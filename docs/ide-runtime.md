...
New provider layers use helper revision 4 and bundled `sigil.sigil-activity` version 0.3.0. The source hash remains part of the immutable image key. Each start copies the current extension version into the provider-only profile. When no user settings exist, the provider seeds a Bash profile if `/bin/bash` is available; machine caller environments are unchanged. Existing settings remain intact. A disabled or old activity extension cannot establish the required `terminal-observation-v4` status contract.
...
### E review correction: immediate conflict and bounded producer

The v4 authority status contract requires producer observer IDs prefixed `v3:`. The new helper cannot advertise ready from the unbounded 0.2.0 extension. A late old observer after a current observer is conflicting custody. A different valid observer immediately latches `conflict`; even a later fresh empty snapshot from the original observer cannot clear it. Time never clears conflict. Only the supported owned-stop/new-start custody path resets the observer. Missing observer traffic remains missing/stale as before.

Same-observer duplicate/out-of-order sequences are benign and do not renew activity. A genuinely newer invalid or over-capacity snapshot immediately marks `incomplete`, preserves known commands and advances the sequence watermark; a delayed complete snapshot cannot undo that evidence. Only a strictly newer complete observation from that same nonconflicted observer can restore completeness.

The extension checks the 256-terminal limit before walking the VS Code terminal array. It retains at most 4096 active execution entries and 256 terminal identities for lost-execution recovery; startup uncertainty uses weak references. On overflow it sends a small existing-shape snapshot with the reserved terminal ID `observation-overflow`, `integrated:false`, no execution list entries and no user event. The reserved ID cannot collide with generated `v3:<UUID>:terminal:<sequence>` identities. The helper preserves known commands and marks incomplete. Crucially, a v2 helper can deserialize this unchanged shape and sees an unsupported terminal, so overflow is never an apparently empty idle snapshot. The v4 controller additionally rejects v2 and v3 helper idle/stop/finish authority.

Returning below the terminal limit marks remaining terminals unobserved: actual execution evidence or closure must establish their state. Execution overflow stays uncertain until the affected terminal closes; an unknown end cannot discharge an identity that was never retained. If the bounded recovery-identity set itself is exhausted, uncertainty stays latched for operator custody recovery. Repeated overflow heartbeats do not retain more objects, serialize growing arrays or renew user activity.

The shared production installer copies 0.3.0 into the existing provider profile without changing settings or unrelated extensions. The focused pinned-provider fixture seeds the exact old 0.2.0 package/source from commit46f1e1b, uses this installer, and requires actual v3 observation. Disabled-extension preferences are preserved and remain fail-closed; the fixture does not prove deployment image startup or live operator recovery. Updated source hashes and helper revision are required in the reviewed image, and a running old companion must not be silently adopted.


The attempted actual 257-pseudoterminal stress in fix round1 established incomplete observation and refused finish, but emitted active-phase upstream listener-leak diagnostics and did not capture the expected recovery status before failing. It is not a recovery pass. The focused producer close/execution transcript verifies the defined recovery semantics; a separate upgrade-only pinned-browser run covers old-profile activation and normal supported-shell/unsupported-shell closure. No diagnostics exception was widened for the stress attempt.


### E fix round2: retain custody before rejecting an old producer

Authority is now helper revision4 / `terminal-observation-v4`. The bounded producer is unchanged: activity extension0.3.0, `v3:<UUID>` observer identities and the same legacy-readable overflow shape. A matching running helper authority is required before cleanup; v3 helper status may still be read, but cannot authorize idle cleanup, stop or finish. The new source hash remains part of image identity.

A syntactically valid first old observer now binds its single bounded identity and sequence watermark before version rejection. Its unsupported telemetry remains incomplete, preserving prior known commands and custody of any unobserved work described by that observer. It never establishes readiness, even when its own later report is empty. A different v3 producer immediately conflicts; repeated new-producer heartbeats, delayed old traffic and time cannot release that custody. No unbounded command/observer collection or automatic upgrade takeover was introduced.

This conservative rule also covers an old first snapshot with no executions: rejected version telemetry is not proof of writer quiescence. Recovery requires the supported exact-custody operator path with prior writers verified stopped; selecting/installing a newer extension alone is insufficient. Status and files stay available while destructive operations remain blocked.

The prior actual old-profile browser run observed authorityv3 with the previously recorded extension bytes; it remains prior-version evidence, not a new authorityv4 browser execution or evidence of overlapping old-writer quiescence. The unchanged producer/Node/provider suites were not repeated for this helper custody correction and status-label change. Focused old-first state-machine and real helper/controller tests, old-v3-helper refusal, formatting and affected Clippy provide this round's source evidence.
