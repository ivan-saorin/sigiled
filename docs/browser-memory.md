...
The common responsive shell has an index rail, filtered/paginated result list and details. Selection and filters are in the URL; small screens use a focused detail panel and Back to results. Imported content and source metadata are text nodes. Only safe HTTP(S) references without userinfo become links. An indexed source revision change preserves displayed text/draft until Review updated source. Source edits are available only through the verified D3 association below.
...
Project Memory tabs show durable setup status and a namespace link only after confirmed registry ownership. D3's accepted-document contract and fixed trusted validation are described in [memory-enrollment.md](memory-enrollment.md). Exact imported source identity, content SHA, live ownership capability and actual canonical repository ancestry must match before Edit source calls C2 POST `/browser/api/projects/{project}/ide` with `{idempotency_key,target:{path}}`. Indexed commit and current accepted commit remain separate; C2 checks current checkout/file readiness on launch and reports moved/missing/setup failures. Accepted documents use authenticated project Retry, never the legacy recorded-source Git reindex fallback. D2 draft recovery, original-actor binding and inert unsafe references remain unchanged.
...
D3 review fix 1: source-edit ownership is refreshed separately from retained text/annotation drafts and revoked on missing/failed detail. Every deliberate Edit source re-fetches the selected index/document and compares owner/project/path before passing the current verified target to C2. A stale control cannot authorize a captured target. Ingestion status uses the persisted effective namespace, and shared-source guards apply only to mem0.


## Encoded response compatibility (Stage E)

Curation readiness additionally requires live `response_budget: "encoded-pages-v1"`. Deploy the matching Memory producer before enabling the updated browser curation controls; older Memory reads remain compatible where their response fits, while new curation controls report update/auth setup required. Accepted enrollment's `sigil-accepted-v1` and fixed original-caller verifier requirements remain separate and unchanged.

History and browse use bounded complete-item pages targeting 1 MiB of encoded JSON; cursors continue without dropping the item that reached the budget. A single accumulated curation row can exceed that page target. The adapter allows up to 8 MiB on chunk/browse/search/curation and single-manual detail routes, retains 2 MiB elsewhere, checks both declared and streamed lengths, and never truncates text/annotations. Unbounded legacy metadata beyond those finite limits remains explicitly unavailable. Search reports `candidate_count` and `response_limited`; it remains one bounded candidate search, with Browse providing complete traversal instead of rerunning inference to fabricate pagination.

Stage E joins exact actual Memory handler bodies to typed adapter projections and authored manual-save, contextual annotation and preview-confirmed forget controls. Actual accepted SDE dossier bytes pass native handoff, normal accepted-master Git checks, controller snapshot/authorization proof, real Memory SQLite/Lance/Fake embedding, fresh namespace/repository association and actual C2 file-target/missing/restored probes. The transport between separately hosted test processes replays immutable synthetic handler bodies; it is not a live cross-service/edge deployment proof. Indexed commit and current checkout remain explicitly distinct.

## Final review correction: extraction uses the declared wire budget

The combined browser router now applies only its exact method/path body boundary, so a valid manual JSON request above 16 KiB reaches the existing Memory handler. The manual wire budget remains 450,000 bytes and decoded text remains limited to 65,536 UTF-8 bytes; other Memory mutation budgets, field validation, authentication, origin and CSRF checks are unchanged. A combined-router fixture verifies a 20,000-byte manual save through a local fake Memory service and rejects a request one byte above the existing wire budget before forwarding.
