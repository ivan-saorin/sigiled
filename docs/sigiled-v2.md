...
| DEC-28 | **Composed-service authentication: an IdP identity is a stack credential.** Raised by `sde` (the first composed service), which measured the wall from a session container: every service edge-gated by a static bearer, a driver JWT refused everywhere, *a service's public spec unreadable from where its client is written* (`sde` `docs/composed-service-auth.md`). The machine gate at the edge accepts **two credentials**: the static `API_BEARER_KEY` (matched first, zero-cost, unchanged for every existing caller — deleted when nothing presents it) **or** any Bearer adjudicated by `GET\|POST /sigiled/auth/verify` via Caddy `forward_auth` — sigiledd validates RS256 signature, expiry and issuer against the stack IdP's JWKS with the machinery it already ran for drivers (no `xcaddy` build, no second crypto). Policy is `SIGILED_SERVICE_POLICY`: **`any-authenticated` (default, the operator's chosen posture)** — a genuine stack identity suffices; the trust boundary is the IdP's user set, same as the shared bearer it replaces, minus the shared secret, plus a named caller, per-client revocation and hourly rotation — or **`per-service`**, the tested, documented, off-by-default tightening: caller must carry `svc:<service>` (group, NOT audience — authentik audiences are per-provider, so `aud=<callee>` would need one provider per caller×callee pair; groups ride the existing `sigiled-groups` mapping), provisioned from the manifest's declared dependency graph `[compose] services = […]`. A misspelled policy value **panics at boot** — a security setting silently resolving to the permissive mode is worse than none. The edge passes the callee name from the hostname (`{http.request.host.labels.2}` — snippet args proved unreliable on the deployed caddy) so the tightening is one env var, never an edge edit. How-to: `docs/per-service-authorization.md`. Under per-service, sessions additionally need a project-scoped token (a driver JWT names the driver, never the project) — the token-broker design is recorded there as the piece to build first *if* that day comes. **Proposed and amended (per-callee → any-authenticated default) 2026-08-15; ratified by il Re 2026-08-15 (in chat); deployed to the live edge and verified end-to-end the same evening** — driver JWT reads `genie`/`adhd` `openapi.json` and `search` JSON through the gate; static callers regression-checked green. |
