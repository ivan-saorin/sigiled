...
## Final review correction: request encoding and retained accepted drafts

Research creation has a 394,631-byte encoded JSON ceiling, derived from both independent 32,768-byte UTF-8 text fields, the 128-byte operation identifier, all ten accepted field names, bounded scalar options and punctuation. The bound allows six-byte JSON escapes for every decoded byte, including keys. Extra whitespace and alternate scalar encodings must fit this same finite wire ceiling. Unknown fields remain rejected. The outer method/path boundary is the single body-budget authority; the JSON extractor no longer adds an incompatible 16 KiB cap. Stage submissions retain their 262,144-byte budget. Unknown routes remain unavailable, and bodyless routes still reject bodies. Authentication, exact origin and CSRF requirements are unchanged.

An accepted retained draft renders both View run and explicit New research whenever its form returns, including when acceptance completes while the earlier form is detached or after the user has already returned. New research preserves current problem/context/options and creates the fresh operation identity only on that action. An uncertain retry keeps the original identity and frozen submitted payload; newer edits remain unsent. These cases are covered by the real browser assets with local fake services. They do not invoke a model provider.
