...
The embedded dashboard is available at `/` and `/ui/*`. It uses only root `/browser/*` adapters, separate from bearer-only machine routes. `/ui/overview`, `/ui/projects`, `/ui/projects/new`, and `/ui/projects/{project}/{tab}` are stable navigable URLs. Shared Research and Models use the reviewed B3 adapters; Memory uses the D2 browser described in `browser-memory.md`. No live research, model, IDE, memory-curation or deployment operations are claimed.

## Origin and data boundaries

`SIGILED_BROWSER_DASHBOARD_ORIGIN` optionally selects exactly one member of `SIGILED_BROWSER_ORIGINS`; absent, the first explicitly configured origin owns the dashboard. D2 optionally selects a distinct exact `SIGILED_BROWSER_MEMORY_ORIGIN` from that allowlist. Root/assets are served only on those two selected sites; Memory root opens its three-pane browser. Other configured authentication sites still cannot serve assets. See `browser-memory.md` for exact routing and host-only SSO. Existing machine bare and `/sigiled` paths are unchanged. Do not route `/ui/*` to machine `/projects/*`.
