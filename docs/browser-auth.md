...
Login transactions remain in the bounded store during code exchange and verification. A valid binding claims a transaction once; mismatched/absent bindings cannot consume another attempt. Starting a new login with the previous binding invalidates its pending or in-flight generation. Provider completion must atomically confirm that the same generation remains live before publishing credentials or replacing the linked session. Timeout/cancellation/failure removes only that attempt; a superseded callback issues no cookies. Authenticated CSRF-authorized logout cancels every attempt linked to its session, so late code-exchange responses cannot sign the user back in. GET initiation never logs out the established session; replacement occurs only after successful state-bound verification.
