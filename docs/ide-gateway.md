...
The exact origin is `https://{32-hex-session}-g{generation}.ide.example.com`; preview is `https://{32-hex-session}-g{generation}-p{port}.preview.example.com`. Generation is a canonical bounded u64 decimal string in every browser contract, including safe dashboard projections; machine JSON remains numeric. Dashboard values are converted before serialization, preserving the existing 100-item page limits and supported metadata/detail sizes. The 64 KiB helper-operation response bound does not apply to these platform-built projections. The longest generated labels fit DNS limits (54 IDE, 61 preview). Incoming Host is compared to origins constructed from registered records. It cannot choose a container or arbitrary upstream.
...
Feature advertisement, allocation and browser `start` share the same readiness predicate: configured IDE domain and durable state store. An existing owned session cannot bypass this gate to start its provider. Status, checkpoint and deliberate finish remain available as explicit recovery operations when browser IDE activation is disabled; they retain B1 authorization, ownership/generation and C1/A1 checks.
...
Focused Rust fixtures cover real HTTP/WS transports, fake B1 credentials/refresh, generation strings above 2^53, single-use tickets, origin/CSRF, owner isolation, previews, debt/intent recovery and normal failed-close retry. The ignored `pinned_provider_gateway_browser_smoke` uses actual pinned provider/helper plus a contained Chromium TLS edge and synthetic session, with owned listener cleanup. It verifies the asserted editor resources, dedicated WebWorker and service worker, exact WSS authority, file line/column, login activity, sandboxed resource handling, isolated preview and sibling HTTP/WS denial. The activity observer is installed before trusted input and requires an actual204 response; the workbench consumes its empty body so the fetch completes. Unexpected HTTP, console and request failures fail the smoke. Exceptions identify the exact method/URL/status or console message for deliberate boundary probes and observed absent VSDA assets; they do not permit activity aborts or arbitrary navigation/teardown failures. `sigiledd/tests/ide-dashboard.cjs` covers actual dashboard asset draft/popup/auth recovery. Existing B2 race tests remain applicable. The smoke's safe status can say provider unavailable because its synthetic fixture does not use Docker's canonical container DNS; its editor transport uses a test-only loopback mapping.

Live release still requires actual wildcard DNS/TLS, IdP compatibility and refresh/logout, pinned bundle/image, supported repository protection/host merge transport, files/search/terminal/Git/extensions, live socket revocation and persistence/recovery checks. Provider VSDA JS/WASM resource404s and the resulting strict-MIME script refusal were observed while the asserted editor, workers, WSS and file navigation succeeded. Their bundle/feature impact remains a live/release compatibility investigation; this smoke does not establish universal provider-resource compatibility or a clean console. The pre-existing repository subprocess `termination_unconfirmed` diagnostic remains qualified and is deferred to Stage E; gateway tests do not change that supervisor or its budgets. No live Docker, SSO, policy, lifecycle, paid inference or deployment was performed for C2.
