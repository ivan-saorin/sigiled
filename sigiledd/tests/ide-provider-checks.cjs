...
class Diagnostics {
 ...
 expectConsole(text,url=null){assert(url||/https?:\/\/|wss?:\/\//.test(text),"console exception must identify its exact URL");this.expectedConsole.add(JSON.stringify([text,url]));}
 expectNetwork(url,error,{method='GET',status=null,resourceType=null}={}){this.expectedNetwork.set(method+' '+url+' '+error,{status,resourceType});}
 check(){
  ...
  const badConsole=this.console.filter(r=>!this.expectedConsole.has(JSON.stringify([r.text,r.url]))&&!this.expectedConsole.has(JSON.stringify([r.text,null]))&&!Object.entries(phrases).some(([status,phrase])=>this.http.some(h=>h.url===r.url&&h.status===Number(status)&&this.expectedHttp.get(h.method+' '+h.url)===h.status)&&r.text===`Failed to load resource: the server responded with a status of ${status} (${phrase})`));
  const badNetwork=this.network.filter(r=>{const expected=this.expectedNetwork.get(r.method+' '+r.url+' '+r.error);return !expected || (expected.resourceType!==null&&expected.resourceType!==r.resourceType) || (expected.status!==null&&!this.http.some(h=>h.url===r.url&&h.method===r.method&&h.status===expected.status));});
