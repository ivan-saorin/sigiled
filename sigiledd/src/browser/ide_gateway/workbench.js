...
(async()=>{
  ...
  async function request(path,body){const r=await fetch(path,{method:body?'POST':'GET',credentials:'same-origin',cache:'no-store',headers:body?{'Content-Type':'application/json','X-Sigil-CSRF':session.csrf_token}:{},body:body?JSON.stringify(body):undefined});let data=r.status===204?{}:await r.json();if(!r.ok){const e=new Error((data.error||'Workspace unavailable').replaceAll('_',' ')+'. Your work is preserved.');e.status=r.status;throw e;}return data;}
