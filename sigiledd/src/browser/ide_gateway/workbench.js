...
(async()=>{
  ...
  async function inspect(){try{const s=await request('/_sigil/status');const p=s.provider?.observed||s.provider;status.textContent=p?.activity_contract!=='terminal-observation-v2' || !['ready','not_required'].includes(p?.activity_observation)?'Terminal activity is uncertain. Close unsupported terminals or recover the editor before finishing.':p?.durability?.state==='pushed'?'Saved files pushed. Unsaved editor buffers remain in the editor.':(p?.error||p?.state||'Workspace open').replaceAll('_',' ');}catch(e){message(e);}}
